...
struct WatchlistView: View {
    ...
    private let quoteRefreshTimer = Timer.publish(every: 60, on: .main, in: .common).autoconnect()
    ...
    private var selectedSymbols: [String] {
        selectedWatchlist?.symbols.map { normalizeSymbol($0) } ?? []
    }
...
private struct SmartSymbol: Identifiable, Hashable {
    ...
    static let known: [SmartSymbol] = [
        ...
        .init(title: "Nasdaq Futures", symbol: "NQ=F", aliases: ["NQ", "NAS100", "NASDAQ", "NQ=F"]),
        .init(title: "S&P Futures", symbol: "ES=F", aliases: ["ES", "SP500", "S&P", "ES=F"]),
